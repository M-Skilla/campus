...
public class SuggestionMessageAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private static final int VIEW_TYPE_STUDENT = 0;
    private static final int VIEW_TYPE_STAFF = 1;

    private List<SuggestionMessage> messages;

    public SuggestionMessageAdapter(List<SuggestionMessage> messages) {
        this.messages = messages;
    }

    @Override
    public int getItemViewType(int position) {
        SuggestionMessage message = messages.get(position);
    ...
    // ViewHolder for student messages (right-aligned, blue bubbles)
    static class StudentMessageViewHolder extends RecyclerView.ViewHolder {
        ...
        public void bind(SuggestionMessage message) {
            tvMessage.setText(message.getText());
            tvTimestamp.setText(message.getFormattedTime());

            if (message.hasAttachment()) {
                ivAttachment.setVisibility(View.VISIBLE);
                // You can add attachment preview logic here
            } else {
    ...
    // ViewHolder for staff messages (left-aligned, gray bubbles)
